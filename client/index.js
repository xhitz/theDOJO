...
const initApp = () => {
    ...
    web3.eth.getAccounts()
        ...
        .then(result => {
            mainIII6.methods.amIuser(accounts[0]).call().then(logCheck => {
                logged = logCheck;
                console.log(logged);
                if (logged) {
                    ...
                    if (result[4] > 11) $pubInput.checked = true;
                    else $pubInput.checked = false;
                    if (result[4] > 22) $advInput.checked = true;
                    else $advInput.checked = false;
                    if (result[4] > 55) $admInput.checked = true;
                    else {
                        $admInput.checked = false;
                        $admInput.visibility = "hidden";
                    }
                }
                else { $userForm.outerHTML = ""; $account.innerHTML = "Please Log In with a MetaMask" }
            });
